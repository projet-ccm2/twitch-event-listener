    ...
    router.post('/admin/channels', async (req, res) => {
        try {
            ...
            if (!twitch_user_id || !login) {
                return res.status(400).json({ error: 'twitch_user_id and login are required' });
            }
            const newChannel: ChannelConfig = {
                twitch_user_id: twitch_user_id,
                login: login,
                scopes: body.scopes || [],
                listen_eventsub: body.listen_eventsub !== undefined ? !!body.listen_eventsub : true,
                listen_chat_irc: body.listen_chat_irc !== undefined ? !!body.listen_chat_irc : false,
