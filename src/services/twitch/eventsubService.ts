...
export class EventSubService {
    ...
    public async handleWebhook(req: Request, res: Response) {
        ...
        if (!this.verifySignature(signature, messageId, timestamp, bodyString, envConfig.twitch.webhookSecret)) {
    ...
    private normalizeEvent(payload: any): import('../../models/event').TwitchEvent {
        const subscription = payload.subscription || {};
        const event = payload.event || {};
        const id = `${subscription.id || ''}:${event.id || Date.now()}`;
        const type = subscription.type || 'unknown';
        const channelId = event.broadcaster_user_id || subscription.condition?.broadcaster_user_id || 'unknown';
    ...
    private async subscribeToTopic(
        ...
        topicConfig: string | { name: string; version?: string; condition?: Record<string, string> },
        clientId: string,
        ...
        if (typeof topicConfig === 'string') {
            ...
            if (topicName === 'channel.follow') {
                version = '2';
                condition = { broadcaster_user_id: channel.twitch_user_id, moderator_user_id: channel.twitch_user_id };
            ...
            condition = topicConfig.condition || { broadcaster_user_id: channel.twitch_user_id };
        }
                ...
                const req = https.request(options, (res) => {
                    ...
                    res.on('end', () => {
                        if (res.statusCode && res.statusCode >= 200 && res.statusCode < 300) {
                            logger.info(`Subscribed to ${topicName} for ${channel.login}`, { service: 'twitch-eventsub' });
                        } else if (res.statusCode === 409) {
                            logger.info(`Subscription already exists for ${topicName} on ${channel.login}`, { service: 'twitch-eventsub' });
                        } else {
                            logger.warn(`Failed to subscribe to ${topicName}: ${res.statusCode} ${data}`, { service: 'twitch-eventsub' });
                        }
                        resolve();
                    });
                });
