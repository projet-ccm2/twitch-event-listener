...
describe('loggerMiddleware', () => {
    ...
    beforeEach(() => {
        hrSpy = jest.spyOn(process, 'hrtime').mockImplementation((time?: [number, number]) => {
    ...
    test('logs request details on finish', () => {
        ...
        expect(logger.info).toHaveBeenCalledWith(expect.stringContaining('GET /metrics 200 - 5.00 ms'));
    });
});
