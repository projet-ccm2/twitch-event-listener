...
export class IrcService {
    ...
    public connect() {
        ...
        this.ws.on('close', () => {
            logger.warn('Twitch IRC connection closed. Reconnecting in 5s...', { service: 'twitch-irc' });
        ...
        this.ws.on('error', (err) => {
            logger.error('Twitch IRC connection error', { service: 'twitch-irc', error: err });
        });
    }
    ...
    public updateSubscriptions() {
        ...
        for (const channel of channels) {
            if (channel.listen_chat_irc && !this.joinedChannels.has(channel.login)) {
                ...
                logger.info(`Joined IRC channel #${channel.login}`, { service: 'twitch-irc' });
            }
        }
    }
