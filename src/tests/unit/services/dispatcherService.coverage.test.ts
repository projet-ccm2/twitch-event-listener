...
describe("DispatcherService Coverage", () => {
    ...
    test("logs to console in development mode (single event)", async () => {
        ...
        await svc.dispatch({ id: "1", type: "test", source: "test", timestamp: "now", version: "1", payload: {} });

        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("[DEV MODE] Event Received"));
    ...
    test("logs to console in development mode (batch)", async () => {
        ...
        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("[DEV MODE] Batch"));
        consoleSpy.mockRestore();
    });
    ...
    test("handles ECONNREFUSED specifically", async () => {
        ...
        const promise = svc.dispatch({ id: "1", type: "test", source: "test", timestamp: "now", version: "1", payload: {} });
    ...
    test("final drop guard logs error if async logic hangs or fails silently", async () => {
        ...
        const promise = svc.dispatch({ id: "guard", type: "test", source: "test", timestamp: "now", version: "1", payload: {} });
